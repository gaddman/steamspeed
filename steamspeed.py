...
parser.add_argument('-a', help='Application ID', default='232370') # Steam game ID. https://store.steampowered.com/app/xxx
...
download = 'steamcmd +login ' + user + ' ' + passwd + ' +@sSteamCmdForcePlatformType windows +app_uninstall -complete ' + appID + ' +app_update ' + appID + ' +download_sources +quit'
...
print('Download test for application {} [{}] (also see https://steamdb.info/app/{}/)'.format(appID,appName,appID))
...
while True:
	...
	if line:
		...
		if 'Update Required,Fully Installed,Files Missing,Uninstalling' in line:
			...
			delta = (datetime.strptime(endtime, '%H:%M:%S')-datetime.strptime(starttime, '%H:%M:%S')).seconds
			break
		elif 'Failed' in line:
...
print('Completed download of {:d} MB in {:d} seconds at {:d} Mbps ({:0.1f} MB/s), from {:s} to {:s}'.format(int(size/1024/1024),delta,int(speed),speed/8,starttime,endtime))
